...
{
    public class PSWDPropertyArgumentTransformationAttribute : ArgumentTransformationAttribute
    {
        private object ConvertArgumentElement(SessionState sessionState, object element)
        {
            if (element is string || element is PSObject && ((PSObject)element).BaseObject is string)
            {
                ...
                Property property = (Property)provider.GetEntityFromId(new EntityId(qId));
                if (property != null)
                {
                    return new PSWDProperty(property);
                }
                else
                {
                    return element;
